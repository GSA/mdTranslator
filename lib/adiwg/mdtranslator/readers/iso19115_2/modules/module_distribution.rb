            ...
            module Distribution
               ...
               def self.unpack(xDistInfo, hResponseObj)
                  ...
                  if hDistribution[:distributor][0][:transferOptions].empty?
                     hDistribution[:distributor][0][:transferOptions] = xTransfers.map { |t| Transfer.unpack(t, hResponseObj) }
                  end
                  ...
                  unless xDistFormat.empty?
                     ...
                     unless distributionFormats.compact.empty?
                        unless hDistribution[:distributor][0][:transferOptions].compact.empty?
                           hDistribution[:distributor][0][:transferOptions].map.with_index { |opts, idx |
